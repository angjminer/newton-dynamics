...
class ndShapeConvexHull::ndConvexBox
{
	...
	#define D_CONVEX_VERTEX_SPLIT_BOX			16
	...
	bool ndShapeConvexHull::Create(ndInt32 count, ndInt32 strideInBytes, const ndFloat32* const vertexArray, ndFloat32 tolerance, ndInt32 maxPointsOut)
	{
		...
		if (vertexCount > D_CONVEX_VERTEX_BRUTE_FORCE_SPLIT)
		{
			...
			for (ndInt32 i = 0; i < boxCount; ++i)
			{
				...
				if (box->m_leftBox == -1)
				{
					...
					ndInt32 soaCount = ((box->m_vertexCount + 7) & -8) / 8;
					m_soaVertexCount += soaCount;
				}
			}

			m_soa_x = (ndVector8*)ndMemory::Malloc(m_soaVertexCount * sizeof(ndVector8));
			m_soa_y = (ndVector8*)ndMemory::Malloc(m_soaVertexCount * sizeof(ndVector8));
			m_soa_z = (ndVector8*)ndMemory::Malloc(m_soaVertexCount * sizeof(ndVector8));
			m_soa_index = (ndVector8*)ndMemory::Malloc(m_soaVertexCount * sizeof(ndVector8));
			...
			for (ndInt32 k = 0; k < boxCount; ++k)
			{
				...
				{
					ndAssert(box->m_rightBox == -1);
					const ndInt32 soaCount = ((box->m_vertexCount + 7) & -8) / 8;

					ndFloat32* const indexptr = &m_soa_index[startAcc][0];
					for (ndInt32 i = 0; i < box->m_vertexCount; ++i)
					{
						array[i] = m_vertex[box->m_vertexStart + i];
						indexptr[i] = ndFloat32(box->m_vertexStart + i);
					}
					for (ndInt32 i = box->m_vertexCount; i < soaCount * 8; ++i)
					{
						array[i] = m_vertex[box->m_vertexStart];
						indexptr[i] = ndFloat32(box->m_vertexStart);
					}

					ndVector* const soa_x = (ndVector*)m_soa_x;
					ndVector* const soa_y = (ndVector*)m_soa_y;
					ndVector* const soa_z = (ndVector*)m_soa_z;
					for (ndInt32 i = 0; i < box->m_vertexCount; i += 8)
					{
						ndVector temp;
						ndInt32 j = startAcc + i / 8;
						ndVector::Transpose4x4(
							soa_x[j * 2], soa_y[j * 2], soa_z[j * 2], temp,
							array[i + 0], array[i + 1], array[i + 2], array[i + 3]);
						ndVector::Transpose4x4(
							soa_x[j * 2 + 1], soa_y[j * 2 + 1], soa_z[j * 2 + 1], temp,
		...
		else
		{
			...
			ndFloat32* const indexptr = &m_soa_index[0][0];
			for (ndInt32 i = 0; i < m_vertexCount; ++i)
			{
				array[i] = m_vertex[i];
			...
			for (ndInt32 i = m_vertexCount; i < m_soaVertexCount * 8; ++i)
			{
				array[i] = m_vertex[0];
			...
			for (ndInt32 i = 0; i < m_soaVertexCount; ++i)
			{
				...
				ndInt32 j = i * 8;
				ndVector::Transpose4x4(
					soa_x[i * 2], soa_y[i * 2], soa_z[i * 2], temp,
					array[j + 0], array[j + 1], array[j + 2], array[j + 3]);
				ndVector::Transpose4x4(
					soa_x[i * 2 + 1], soa_y[i * 2 + 1], soa_z[i * 2 + 1], temp,
	...
	{
		const ndVector8 dirX(dir.m_x);
		const ndVector8 dirY(dir.m_y);
		const ndVector8 dirZ(dir.m_z);

		ndVector8 support(m_soa_index[0]);
		ndVector8 maxProj(m_soa_x[0] * dirX + m_soa_y[0] * dirY + m_soa_z[0] * dirZ);
		for (ndInt32 i = 1; i < m_soaVertexCount; ++i)
		{
			const ndVector8 dot(m_soa_x[i] * dirX + m_soa_y[i] * dirY + m_soa_z[i] * dirZ);
			support = support.Select(m_soa_index[i], dot > maxProj);
			maxProj = maxProj.GetMax(dot);
		}

		// reduce for 8 to 4
		const ndVector dotLow(maxProj.GetLow());
		const ndVector dotHigh(maxProj.GetHigh());
		const ndVector support4(support.GetLow().Select(support.GetHigh(), dotHigh > dotLow));
		const ndVector dot4(dotLow.GetMax(dotHigh));

		// reduce for 4 to 2
		const ndVector dot2(dot4.ShiftRight().ShiftRight());
		const ndVector support1(support4.ShiftRight().ShiftRight());
		const ndVector support2(support4.Select(support1, dot2 > dot4));
		const ndVector dot1(dot2.GetMax(dot4));

		// reduce for 2 to 1
		const ndVector dot0(dot1.ShiftRight());
		const ndVector support0(support2.ShiftRight());
		const ndVector supportIndex(support2.Select(support0, dot0 > dot1));

		const ndInt32 index = ndInt32(supportIndex.GetScalar());
		if (vertexIndex)
		{
			*vertexIndex = index;
		}
	...
	ndVector ndShapeConvexHull::SupportVertexHierarchical(const ndVector& dir, ndInt32* const vertexIndex) const
	{
		...
		ndVector8 support(ndFloat32(-1.0f));
		//ndVector8 maxProj(ndFloat32(-1.0e20f));
		ndVector8 maxVertexProjection(maxProj);
			...
			if (dist > maxProj)
			{
				...
				else
				{
					for (ndInt32 j = box.m_soaVertexCount - 1; j >= 0; --j)
					{
						...
						ndVector8 dot(m_soa_x[i] * dirX + m_soa_y[i] * dirY + m_soa_z[i] * dirZ);
					...
					const ndVector maxProj4(maxVertexProjection.GetLow().GetMax(maxVertexProjection.GetHigh()));
					const ndVector maxProj2(maxProj4.GetMax(maxProj4.ShiftRight().ShiftRight()));
					const ndVector maxProj1(maxProj2.GetMax(maxProj2.ShiftRight()));
					maxProj = ndMax(maxProj, maxProj1.GetScalar());
				}
			}
		}

		// reduce for 8 to 4
		const ndVector dotLow(maxVertexProjection.GetLow());
		const ndVector dotHigh(maxVertexProjection.GetHigh());
		const ndVector support4(support.GetLow().Select(support.GetHigh(), dotHigh > dotLow));
		const ndVector dot4(dotLow.GetMax(dotHigh));
